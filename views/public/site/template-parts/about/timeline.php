		...
		<?php foreach ($items as $i => $item) : ?>
			...
			<div data-tab-panel role="tabpanel" class="<?php echo 0 === $i ? '' : 'hidden'; ?>">
				<div class="relative rounded-3xl overflow-hidden min-h-[320px] md:min-h-[420px]">
					<?php if ($image !== '') : ?>
						<img src="<?php echo esc_url(vr_media_url($image)); ?>" alt="<?php echo esc_attr($title); ?>" class="absolute inset-0 w-full h-full object-cover" decoding="async" loading="lazy"<?php $__ss = vr_srcset($image); if ($__ss !== '') : ?> srcset="<?php echo esc_attr($__ss); ?>" sizes="(max-width: 768px) 100vw, 50vw"<?php endif; ?> />
					<?php endif; ?>
