<?php
/**
 * Intermediate + NEET program
 *
 * @package VR_Doctors
 */

$s        = vr_get_page_section('courses', 'intermediate');
$features = !empty($s['features']) && is_array($s['features']) ? $s['features'] : array();
?>
<section id="intermediate + neet" class="py-12 md:py-16 bg-white scroll-mt-24">
		...
		<div class="order-1 md:order-2">
			...
			<ul class="mt-6 space-y-3">
				<?php foreach ($features as $f) : ?>
					<li class="flex items-start gap-2 text-gray-700"><span class="text-orange-500 font-bold">✓</span> <?php echo esc_html($f); ?></li>
				<?php endforeach; ?>
			</ul>
			<a href="<?php echo vr_esc_url($s['cta_url']); ?>" class="inline-block mt-8 bg-gradient-to-r from-orange-500 to-orange-600 text-white px-6 py-3 rounded-xl font-semibold hover:opacity-90 transition">
