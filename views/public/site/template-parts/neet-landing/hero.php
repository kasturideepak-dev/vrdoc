...
$hero  = vr_get_page_section($slug, 'hero');
	...
	<div class="max-w-6xl mx-auto px-4 md:px-6 relative z-10">
		<div class="grid lg:grid-cols-[1.2fr_0.8fr] gap-8 lg:gap-10 items-start">
			<div class="max-w-3xl">
				...
				<h1 class="mt-4 text-3xl md:text-4xl lg:text-5xl font-bold text-white leading-tight"><?php echo esc_html($hero['title']); ?></h1>
				<p class="mt-5 text-sm md:text-base text-blue-50 leading-relaxed"><?php echo esc_html($hero['paragraph_1']); ?></p>
				<p class="mt-4 text-sm md:text-base text-blue-100 leading-relaxed"><?php echo esc_html($hero['paragraph_2']); ?></p>
				<div class="mt-6 flex flex-wrap gap-3">
					<a href="#hero-enquiry" class="inline-flex items-center justify-center bg-gradient-to-r from-orange-500 to-orange-600 text-white px-6 py-3 rounded-xl font-semibold text-sm md:text-base shadow-lg hover:opacity-90 transition lg:hidden"><?php echo esc_html($hero['primary_cta_text']); ?></a>
					<a href="<?php echo vr_esc_url($hero['secondary_cta_url'] ?: $phone); ?>" class="inline-flex items-center justify-center border border-white/40 text-white px-6 py-3 rounded-xl font-semibold text-sm md:text-base hover:bg-white hover:text-blue-900 transition"><?php echo esc_html($hero['secondary_cta_text']); ?></a>
				</div>
				<div class="mt-5 flex flex-col sm:flex-row sm:flex-wrap gap-3 sm:gap-5 text-sm md:text-base text-blue-100">
					<a href="<?php echo vr_esc_url($phone); ?>" class="inline-flex items-center gap-2 hover:text-orange-300 transition"><i class="fa-solid fa-phone text-orange-400"></i><span><?php echo esc_html($hero['phone_display']); ?></span></a>
					<a href="mailto:<?php echo esc_attr($email); ?>" class="inline-flex items-center gap-2 hover:text-orange-300 transition"><i class="fa-solid fa-envelope text-orange-400"></i><span><?php echo esc_html($email); ?></span></a>
				</div>
			</div>
